...
function config_url {
    ...
    echo "https://dl.cloudsmith.io/public/moodeaudio/m8y/config.deb.txt?$params" | sed 's/ /%20/g'
}
...
EOF
 ...
 >>>>: doesn't work after an hour, please contact moodeaudio
 >>>>: for assistance.
 >>>>:
EOF
        }
...
function import_gpg_key {
    local text="Importing 'moodeaudio/m8y' repository GPG keys ..."
    echo_running "$text"

    local gpg_keyring_path="/usr/share/keyrings/moodeaudio-m8y-archive-keyring.gpg"
    
    curl -1sLf "https://dl.cloudsmith.io/public/moodeaudio/m8y/gpg.E5A251A46C58117E.key" | gpg --dearmor >> $gpg_keyring_path
    ...
    echo_okfail "Checking for apt signed-by key support ..." || {
        mv ${gpg_keyring_path} /etc/apt/trusted.gpg.d/moodeaudio-m8y.gpg
        chmod 644 /etc/apt/trusted.gpg.d/moodeaudio-m8y.gpg
    }
...
function setup_repository {
    local repo_path="/etc/apt/sources.list.d/moodeaudio-m8y.list"
    check_fetch_config

    local text="Installing 'moodeaudio/m8y' repository via apt ..."
    echo_running "$text"
...
echo "Executing the  setup script for the 'moodeaudio/m8y' repository ..."
echo
