        ...
        const searchPromise = apiService
          .aggregatedSearch(
            ...
            newResults => {
              ...
              // 调试日志
              console.log('📦 [搜索结果] 收到新结果:', {
                count: newResults.length,
                sources: [...new Set(newResults.map(r => r.source_name))],
                firstItem: newResults[0]?.vod_name,
                keyword: keyword
              })
              ...
              setSearchRes(prevResults => {
                ...
                // 调试日志
                console.log('🔄 [结果排序] 排序后前5个:', mergedRes.slice(0, 5).map(r => ({
                  name: r.vod_name,
                  source: r.source_name,
                  priority: sourcePriorityMap.get(r.source_code || '')
                })))
